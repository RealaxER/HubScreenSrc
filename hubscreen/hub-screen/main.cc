...
void logic_control(std::vector<uint8_t> buff_vec, const struct mosquitto_message *message){
    if (!hub_screen.buff.ParseFromArray(buff_vec.data(), buff_vec.size())) {
        eventQueue.push(Event(Event::ErrParse, message->topic, buff_vec));
    }else {
        if (hub_screen.buff.receiver == Screen){
            event_handle(message);
        }
        else {
            eventQueue.push(Event(Event::ErrFormat, message->topic, vec_data));
        }
    }
}
